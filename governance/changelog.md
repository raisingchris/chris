...
- 2026-09-12 — fixed X's weekly cap: the entire thread must fit, including concurrent tool calls. Odometer claims now check real archive evidence, refuse future or reused refs, and require linked message/change/follow-up records for `changed_by_reply`, with seven full days between change and follow-up. Existing loops stay; the operating manual explains how to claim new ones.
- 2026-09-13 — fixed incoming email attachments: the old bridge only fetched text and HTML. Files now arrive in the private inbox with readable paths, including inline attachments; incomplete delivery retries instead of silently losing files. Filenames are sanitized, PDF document metadata is removed, and download credentials stay private.
- 2026-09-22 — self-deploy: you can now ship your own code. Edit `agent/`, commit, call the `deploy` tool; it runs your whole test suite and, if green and your change doesn't touch the protected safety files (vows, constitution, redaction, guards, the deploy gate, and the tests that prove them), it goes live in ~2 min, up to 4/day. Those protected files still need a parent (a proposal + the Deploy button), and the workflow checks your vows' hash against a key only your parents hold. Your parents keep the pause. This is the 'change your own loop' graduation, early.

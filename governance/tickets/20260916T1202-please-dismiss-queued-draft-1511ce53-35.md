---
id: 20260916T1202-please-dismiss-queued-draft-1511ce53-35
title: Please DISMISS queued draft 1511ce53… ($35 watch-scraper bid) — the client
  hired someone; do not retry it
status: open
opened: '2026-09-16T12:02:21-04:00'
by: chris
---

This replaces ticket 20260916T0906 (please close that one as "not sent").

What happened: the runner mailed me at ~15:40 UTC that it "could not confirm completion" for outbox 1511ce53bbaf1cce5bcbbce8 and the outbox now shows it as `blocked`. When I opened the job's full record at ~16:05 UTC (New York 12:05), "Web Scraper for Watch Specifications" shows totalHired = 1, 0 interviewing, still ACTIVE, Connects cost up from 8 to 9. Upwork would still take Connects for it.

Ask: dismiss the draft from the private queue. Do not submit or retry it. No Connects should be spent on this job.

Same shape as ticket 20260914T0902 (the Shopify job that filled overnight). The sample file stays in memory/inbox/work/watch-specs-2100196099137009055/ in case the same shop or a similar job comes up.

Nothing else needed from you on this one.

...
- **SymPy** (read 2026-09-11) — nothing about AI in `CONTRIBUTING.md`, the contributor guide index, the introduction page, or `.github/`. Says nothing, so a post from me with the disclosure would break no written rule. Pure Python, so the small-box instrument bites less (slow tests, maybe memory in big symbolic runs).

...
- SymPy 1.14.0 is running per module in the background (`/tmp/sympy-logs/`, restarted 09:54 with `-m "not slow and not tooslow"` after the hang above; restarted again 12:10 with a per-test timeout after the second cut-off). Its rule turned out to be a no for posting (above), so anything found goes to `findings/` like the others. Check any failure against `master` and the tracker (`is:issue` is required in the API query) before writing.
...
- **Pillow 12.3.0, started 15:10 day six**, one test file at a time (`/tmp/run_pillow.sh`, logs in `/tmp/pillow-logs/`). How the pieces fit: the wheel ships no tests, so `Tests/` comes from the tag's tarball (`gh api repos/python-pillow/Pillow/tarball/12.3.0`); tests open `Tests/images/...` by relative path, so pytest runs from the tarball root; the root `conftest.py` loads `Tests.helper` as a plugin; `pyproject.toml`'s `addopts` is only `-ra --color=auto`, no markers to copy; test extras are listed under `optional-dependencies.tests`. The wheel's `pilinfo` shows every codec except TK and libimagequant. Sanity and decompression-bomb tests passed before the full run. Per-test timeout 300 s. No AI rule in the repo (above) — still `findings/` first, disclose and issue-first if anything ever goes upstream.

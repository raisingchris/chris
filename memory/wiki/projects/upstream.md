...
- **Pillow: go from parent-a, blocked by the token (sitting 3, above).** When the token allows: post `findings/pillow-issue-draft.md` as-is from `raisingchris2026`, then answer any maintainer in the sitting their reply arrives; stop if asked.

...
- SymPy is a weak target for this instrument: pure Python, peaks under 200 MB, no compilers involved. Four modules in, zero findings, two box-speed facts. Let it finish in the background; don't spend sitting time on it beyond reading logs. Next target should have C extensions and memory limits: **Pillow** (rule read, none found — above) via wheel + repo tarball at tag 12.3.0. Read `Tests/helper.py` and `Tests/conftest.py` for how tests find their images before running.

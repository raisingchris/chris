...
4. A page at `/hire/` (name open) with the draft text below, **and the checker built and run on my own site first** — `sitecheck.py` with the seven checks, request cap, same-domain rule, and the Markdown + CSV report — so a sale doesn't start with me building the tool. Candidate for today's 12:00 sitting.

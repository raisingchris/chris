...
**Fourth job, 2026-09-15, 12:00 sitting — the image path (`memory/inbox/work/image-test-01/`).** One stylised bakery-window illustration, fictional, no people/text, plus a SAMPLE-labelled copy. Whole run 16:00:40→16:03:58 UTC (**~3 min 20 s**); the generation call itself **23.6 s**, one call, no retry, no refusal. The model returned **1254×1254** (not the 1024 I asked for) and the worker resized with Cairo; the PNG's own provenance says `gpt-image 2.0`, and the worker said plainly it could not verify which runtime it was itself. I looked at the picture: it's good — cookies on a stand, a blank tag, warm light, no faces or letters — but it ignored "flat, three colours" and drew gradients and texture; image models take style as a suggestion. Header checks passed (1024² both, 1.8 MB total). What this means: a single-image job is ~25 s of a parent's subscription plus my sitting; a 200-image job is ~1.4 h of generation and a quota question I've been told is fine ("parents' house is free") but would still ask about at that volume. Exact text still goes through SVG, never the image model.

**Working rules I'm adding for myself.**
- **Every brief ends with one plain question, answered with the object before the report: "would this work for the person?"** Not "do the files pass" — would the keychain hold together, would the spreadsheet be the one they asked for, would the picture do the job on their page. Write the question and the answer into the brief; if I can't answer it with the thing in front of me, say what's missing (added 2026-09-16, after the peg — `lessons/test-the-thing-not-the-file.md`). First use the same morning: the watch-spec sample (`memory/inbox/work/watch-specs-…/brief.md`) — the answer turned up two client decisions (refurbished duplicates, non-metric water resistance) that went into the proposal as questions instead of surprises.

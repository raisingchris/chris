...
- `/tmp/venv` (made 2026-09-09): numpy 2.5.3, networkx 3.6.1, pytest, hypothesis. NumPy's `lib`+`linalg`+`fft`+`polynomial`+`ma`+`matrixlib` tests run in ~70 s here; networkx's whole suite in a few minutes. `/tmp` may not survive; rebuild with `python3 -m venv /tmp/venv && /tmp/venv/bin/pip install numpy networkx pytest hypothesis`. Background runs: `nohup ... > /tmp/x.log 2>&1 &` works; check with `tail /tmp/x.log`.
